...
const EditMemberModal: React.FC<EditMemberModalProps> = ({ member, currentUser, onClose, onSave }) => {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (name && avatar && title && role) {
      const memberData = { name, email, avatar, title, role, incomeSource };
      if (isEditing) {
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            <div className="flex flex-col items-center gap-4">
              <button
                ...
                className="w-24 h-24 rounded-full bg-gray-200 flex items-center justify-center overflow-hidden hover:bg-gray-300 transition-colors ring-2 ring-offset-2 ring-blue-500"
              >
            ...
            <div>
              ...
              <input
                ...
                className="mt-1 block w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-blue-500 focus:border-blue-500"
              />
              <p className="text-xs text-gray-500 mt-1">Se esse membro tiver e-mail, poderá acessar o app futuramente.</p>
            </div>
